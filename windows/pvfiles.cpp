 ...
 * ---------------------------------------------------------------------------
 * $File: //depot/clipka/upov/windows/pvfiles.cpp $
 * $Revision: #5 $
 * $Change: 6009 $
...
void save_povlegal (void)
{
  ...
  fclose (outF) ;
  sprintf (filename, "The " BRANCH_NAME " license was written to the file %sagpl-3.0.txt", DocumentsPath) ;
...
INT_PTR CALLBACK PovLegalDialogProc (HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
  ...
  switch (message)
  {
    case WM_INITDIALOG :
         ...
         SetWindowText (hDlg, BRANCH_NAME " License") ;
