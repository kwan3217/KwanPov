...
*/
        ...
        {
            return ColourChannelIsFinite(mColour[RED]) &&
                   ColourChannelIsFinite(mColour[GREEN]) &&
                   ColourChannelIsFinite(mColour[BLUE]);
        }
...
*/
        ...
        inline bool IsFinite() const
        {
            return (mColour.IsFinite() &&
                    ColourChannelIsFinite(mTransm));
        }
...
/// @relates GenericRGBTColour
template<typename T>
inline GenericRGBTColour<T> Exp(const GenericRGBTColour<T>& a) { return GenericRGBTColour<T>(exp(a.red()), exp(a.green()), exp(a.blue()), exp(a.transm())); }

/// @relates GenericRGBTColour
template<typename T>
inline GenericRGBTColour<T> Sqrt(const GenericRGBTColour<T>& a) { return GenericRGBTColour<T>(sqrt(a.red()), sqrt(a.green()), ::sqrt(a.blue()), sqrt(a.transm())); }
...
*/
        ...
        inline bool IsFinite() const
        {
            bool result = true;
            for (int i = 0; i < channels; i ++)
                result = result && ColourChannelIsFinite(mColour[i]);
            return result;
        }
