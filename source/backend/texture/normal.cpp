...
void Perturb_Normal(Vector3d& Layer_Normal, const TNORMAL *Tnormal, const Vector3d& EPoint, Intersection *Intersection, const Ray *ray, TraceThreadData *Thread)
{
    ...
    Blend_Map = std::dynamic_pointer_cast<NormalBlendMap, GenericNormalBlendMap>(Tnormal->Blend_Map);
    if (Blend_Map != NULL)
    {
    ...
    {
        shared_ptr<SlopeBlendMap> slopeMap = std::dynamic_pointer_cast<SlopeBlendMap, GenericNormalBlendMap>(Tnormal->Blend_Map);
